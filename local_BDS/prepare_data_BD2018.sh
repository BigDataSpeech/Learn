...
;;'
    # Process the STMs
    cat db/$set/stm/*.stm | sort -k1,1 -k2,2 -k4,4n | \
  ...
  # Prepare 'text' file
  cat $dir/stm | grep -v 'inter_segment_gap' | grep -v -e 'ignore_time_segment_in_scoring' -e ';;' | \
  ...
  cat $dir/spk2utt | awk -v set=$set -v pwd=$PWD '{ printf("%s sph2pipe -f wav -p %s/db/%s/sph/%s.sph |\n", $1, pwd, set, $1); }' > $dir/wav.scp
...
#YK  if [ $set == "train" ]; then
#YK    mv data/$set.orig/segments data/$set.orig/segments.temp
#YK    utils/data/extend_segment_times.py --start-padding=0.15 \
#YK      --end-padding=0.1 <data/$set.orig/segments.temp >data/$set.orig/segments || exit 1
#YK    rm data/$set.orig/segments.temp
#YK  fi
